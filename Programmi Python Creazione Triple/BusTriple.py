...
data = json.load( open( path+"/DatiPerElaborazione/BusStation.geojson", "r" ) );
g = Graph();
...
for element in data:
    uri = element["URI"];
    g.add( [URIRef(uri), RDF.type, schema.BusStation ] )
    g.add( [URIRef(uri), RDFS.label, Literal( element["nome"] ) ] )
    g.add( [URIRef(uri), cmo.lineOfPublicTransportSystemStation, Literal( element["linea"], lang='it' ) ] )
...
g.serialize(destination=path+'/Turtles/busStation.ttl', format='turtle')
