...
data = json.load( open( path+"/DatiPerElaborazione/RailwayStation.geojson", "r" ) );
g = Graph();
...
for element in data:
    uri = element["URI"];
    g.add( [URIRef(uri), RDF.type, schema.TrainStation ] )
    g.add( [URIRef(uri), RDFS.label, Literal( element["nome"] ) ] )
    g.add( [URIRef(uri), cmo.organizationOfTransportSystem, Literal( element["gestore"], lang='it' ) ] )
...
g.serialize(destination=path+'/Turtles/railwayStation.ttl', format='turtle')
