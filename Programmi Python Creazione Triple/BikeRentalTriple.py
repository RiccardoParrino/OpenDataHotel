...
data = json.load( open( path + "/DatiPerElaborazione/BikeRental.geojson", "r" ) );
g = Graph();
...
for element in data:
    uri = element["URI"];
    g.add( [URIRef(uri), RDF.type, cmo.BikeRental ] )
...
g.serialize(destination=path+'/Turtles/bikeRental.ttl', format='turtle')
