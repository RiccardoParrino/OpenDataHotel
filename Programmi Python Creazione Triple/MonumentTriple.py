...
for element in data:
    ...
    g.add( [URIRef(uri), cmo.touristAttractionAuthor, Literal( element["autore"] ) ] )
    g.add( [URIRef(uri), cmo.touristAttractionDefinition, Literal( element["definizione"], lang='it') ] )
...
g.serialize(destination=path+'/Turtles/monument.ttl', format='turtle')
