...
data = json.load( open( path+"/DatiPerElaborazione/FastFood.geojson", "r" ) );
g = Graph();
...
for element in data:
    uri = element["URI"];
    g.add( [URIRef(uri), RDF.type, schema.RestaurantFastFood ] )
    ...
    g.add( [URIRef(uri), cmo.deliveryFastFood, Literal( element["delivery"] ) ] )
    g.add( [URIRef(uri), cmo.takeawayFastFood, Literal( element["takeaway"] ) ] )
...
g.serialize(destination=path+'/Turtles/fastfood.ttl', format='turtle')
