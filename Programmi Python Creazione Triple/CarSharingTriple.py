...
data = json.load( open( path + "/DatiPerElaborazione/CarSharing.geojson", "r" ) );
g = Graph();
...
for element in data:
    uri = element["URI"];
    g.add( [URIRef(uri), RDF.type, cmo.CarSharing ] )
...
g.serialize(destination=path + '/Turtles/carSharing.ttl', format='turtle')
