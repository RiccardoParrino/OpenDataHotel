...
inputFile = open( path + "/Hotel/UncleanedHotel.geojson", "r" );
nativeFile = json.load( inputFile );
finalFileName = path + "/DatiPerElaborazione/Hotel.geojson"
definitiveFile = []
temp = {}
counters = {}
stringSet = set()
...
for element in nativeFile:
    stringSet.add( stringa( element["DENOMINAZIONE_STRUTTURA"] ) )

for element in nativeFile:
    element["DENOMINAZIONE_STRUTTURA"] = getMostLikelyWord( stringSet, element["DENOMINAZIONE_STRUTTURA"] );
    
    uri = urify( "hotel",element["DENOMINAZIONE_STRUTTURA"] );
